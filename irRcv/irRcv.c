...
const char IR_BIT_COUNT = 32;

// fully received command (gets overwritten by next one if not read)
static volatile uint32_t irReceivedCommand = 0;

volatile char irRcvErrorCount = 0;
volatile char irRcvErrorBit = 0;
volatile uint32_t irRcvErrorDelta = 0;
...
* @retval IR 32bit command (zero==none)
*/
...
{    
    ...
    // error (must be greater then IR_BIT_COUNT)
    const char ERROR = 99;
    // current bit count
    ...
    // clasify symbol; coding: 0 = 0bit; 1 = 1bit; 2 = start; 3 = unrecognized
    char symbol = (CHECK_SYMBOL(delta,ZERO) ? 0 :
    ...
    switch( symbol )
    {
        ...
        case 1:
            ...
            {
                // good bit (zero or one) and we have space to store it
        ...
        case 2: // start symbol
            // start the command by resetting bit count
            bitCount = 0;
            break;

        default: // symbol timing error (could be the gap between commands)
            if( bitCount < IR_BIT_COUNT && irRcvErrorCount == 0 )
            {
              irRcvErrorBit = bitCount;
              irRcvErrorDelta = delta;
              irRcvErrorCount=1;
            }
            bitCount = ERROR;
            break;
