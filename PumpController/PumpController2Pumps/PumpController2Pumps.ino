...
public:
    ...
    {
        Serial.print("batMV       : ");
        Serial.println(mConfig.batMV);
        Serial.print("batDAC      : ");
        Serial.println(mConfig.batDAC);
        Serial.print("batMinMV    : ");
        Serial.println(mConfig.batMinMV);
        Serial.print("batNomMV    : ");
        Serial.println(mConfig.batNomMV);
        Serial.print("batFloMV    : ");
        Serial.println(mConfig.batFloMV);
        Serial.print("batMaxMV    : ");
        Serial.println(mConfig.batMaxMV);
        Serial.print("pumpTreshold: ");
        Serial.println(mConfig.pumpTreshold);
        Serial.print("pump1OnTime : ");
        Serial.println(mConfig.pump1OnTime);
        Serial.print("pump2OnTime : ");
        Serial.println(mConfig.pump2OnTime);
        Serial.print("pumpRestTime: ");
        Serial.println(mConfig.pumpRestTime);
        Serial.print("sUpdateTime : ");
        Serial.println(mConfig.stateUpdateTime);
    }
...
private:
    ...
    State computeNextState(uint16_t batteryMV) const
    {
        ...
        else
        {
            ...
            case State::CHARGE: // float charging - up to float Volts can switch to is from REST (when no strong dischargin was happening)
                if (batteryMV > config.data().batFloMV)
                    nextState = State::FLOAT;
                break;
            case State::FLOAT:
                if (batteryMV < config.data().batNomMV)
    ...
    static const char *state2str(State state)
    {
        ...
        {
        case State::CHARGE:
            return "CHA"; // battery in normal operating state (charge on, work ok)
        case State::RECOVERY:
            return "REC"; // recovering from deep discharge (leave me alone until reaching NOMinal voltage)
        case State::FLOAT:
            return "FLO"; // not changing, all good for work
        case State::ERROR:
            return "ERROR"; // charging in error (cannot read battery voltage)
        }
        return "????";
    }
...
public:
    enum class State
    {
        ...
        PUMP2,
        REST
    }; // XPUMP1 transitions to REST while PUMP1 (notmal) transitions to PUMP2
...
void printAllState()
{
    ...
    Serial.print(", ");
    Serial.print(pumpStateMachine.currentStateStr());
    Serial.print("/");
    Serial.print(pumpStateMachine.timerCount());
    Serial.print(", Vbat:");
    Serial.print(readBatteryMV());
    Serial.print(", ");
    Serial.print(chargingStateMachine.currentStateStr());
    Serial.print("/");
    Serial.print(chargingStateMachine.timerCount());
    Serial.print(", Vcc:");
    Serial.print(readVccMV());
    Serial.print(", TP:");
    Serial.print(temphum.temp);
    Serial.print(", RH:");
    Serial.print(temphum.rh);
    Serial.print(", TS:");
...
void help()
{
    ...
    Serial.println("c dump configuration");
...
public:
    ...
    bool check()
    {
        ...
        {
            if (hasCmd())
                Serial.println("Resetting stale input");
            reset();
            if (!mLocked)
            {
                mLocked = true;
        ...
        while (Serial.available())
        {
            ...
            if (ch == TERMINATOR)
            {
                if (hasCmd()) // command terminated -> execute
                {
                    Serial.print("cmd ");
                    Serial.print(mCmd);
                    Serial.print(' ');
                    ...
                    if (mCmd == '!' && mPar == 4242)
                    {
                        Serial.println("Unlocking for 30s");
                        mLocked = false;
                    }
                    else if (mLocked)
                    {
                        Serial.println("Locked, !4242 to unlock");
                    }
                    else
                    {
                        if (!executeCmd(mCmd, mPar))
                        {
                            Serial.println("Bad command, ? for help");
                        }
                    }
                }
                reset();
                return false; // command terminated, let the rest run
            }
            if (!addChar(ch))
            {
...
private:
    ...
    bool hasCmd() const { return mCmd > 0; }

    bool addChar(char c)
    {
        return hasCmd() ? addPar(c) : addCmd(c);
    }

    bool expired() const
    {
        return millis() - mTime > 30000; // 30s for collection of all chars the reset and lock it up
    }
...
void setup()
{
    ...
    // throw away the serial input if any for some time (serial wifi monitor boot output)
    inputProcessor.drainInput();

    Serial.println();

    // load or default configuration
    Serial.print("EEPROM configuation...");
    if (!config.load())
    {
        Serial.println("invalid, defaulting.");
        config.reset();
        config.store();
    }
    else
...
void loop()
{
    ...
    bool hadInput = inputProcessor.check();
    if (!hadInput)
    {
