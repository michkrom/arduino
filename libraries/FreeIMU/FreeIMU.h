...
// there should have been external-to-library configuration file
// but for now i define my specific stuff here

// SENSOR_ORIENTATION_CORRECTION(vec) changes orientation of the sensor for your specific applicaton
// raw_values
//   0,1,2 accel x,y,z
//   3,4,5 gyro  x,y,z
//   6,7,8 magn  x,y,z
...
template<class T> void SWAP(T& x,T& y) { T tmp; tmp = x; x = y; y = tmp; }

#define ACC_ORIENTATION_CORRECTION(acc) //SWAP((acc)[0],(acc)[1])
#define GYR_ORIENTATION_CORRECTION(gyr) //SWAP((gyr)[0],(gyr)[1])
#define MAG_ORIENTATION_CORRECTION(mag) //SWAP((mag)[0],(mag)[1])

#define SENSOR_ORIENTATION_CORRECTION(vec)\
{\
    ACC_ORIENTATION_CORRECTION(&vec[0]);\
    GYR_ORIENTATION_CORRECTION(&vec[3]);\
    MAX_ORIENTATION_CORRECTION(&vec[6]);\
\}
...
class FreeIMU
{
  public:
    ...
    static void getEulerRad(const float * const q, float * angles);
