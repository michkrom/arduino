...
// -----------------------------
//
// Based on arduino.cc user "Krodal".
//
//
// Documentation:
...
// --------------------------------------------------------
// helper structure to read all measured values
...
typedef struct
{
  ...
  void swapByteOrder()
  {
    #define SWAP(x) { (x) = ((uint16_t(x) & 0xFF00) >> 8) | ( (uint16_t(x) & 0x00FF) << 8 ); }
    SWAP (x_accel);
    SWAP (y_accel);
    SWAP (z_accel);
    SWAP (x_gyro);
...
// MPU6050_read_all
//
// An extra function to read all measured values
// and correct byte order.
//
int MPU6050_read_all(accel_t_gyro_t * accel_t_gyro);
